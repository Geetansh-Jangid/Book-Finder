...
import arxiv # <<< NEW >>> Import arxiv library
from datetime import datetime # <<< NEW >>> For formatting dates
...
# <<< NEW >>> ArXiv Search Function
async def search_arxiv_async(query: str):
    """Searches ArXiv asynchronously using the python wrapper."""
    print(f"Async Searching ArXiv for: '{query}'...")
    results_list = []
    try:
        search = arxiv.Search(
            query=query,
            ...
            # sort_order = arxiv.SortOrder.Descending # Default is descending
        )

        # The arxiv library's search itself is synchronous, so wrap it
        loop = asyncio.get_running_loop()
        # client = arxiv.Client() # If pagination needed later, use client
        # arxiv_results_generator = await loop.run_in_executor(None, lambda: client.results(search))
        arxiv_results_generator = await loop.run_in_executor(None, search.results)

        for result in arxiv_results_generator:
             # Format authors nicely
             author_names = ", ".join([author.name for author in result.authors])
             # Format published date
             published_date_str = result.published.strftime('%Y-%m-%d') if result.published else 'N/A'
             # Truncate summary for display
             summary_snippet = result.summary[:250] + ('...' if len(result.summary) > 250 else '') # Short summary for preview

             results_list.append({
                "source": "arxiv", # <<< NEW >>> Identifier
                "title": result.title or "N/A",
                "authors": author_names or "N/A",
                "published": published_date_str,
                "summary": summary_snippet or "N/A",
                "pdf_url": result.pdf_url, # Direct PDF link
                "entry_id": result.entry_id, # Unique ID (URL)
                # Add other fields if needed, maintaining some consistency?
                "language": "en", # ArXiv is mostly English
                "extension": "pdf", # ArXiv downloads are PDFs
                "size": "N/A", # Size not readily available from search results
                "mirror1_url": result.pdf_url, # Use pdf_url for consistency in download logic
                "mirror2_url": None,
                "details_url": result.entry_id # Link to the abstract page
             })
        print(f"Found {len(results_list)} results on ArXiv.")
    except Exception as e:
...
async def download_book_to_discord(download_url: str, title: str, extension: str):
    ...
    # <<< MODIFIED >>> Ensure extension is handled correctly, default if needed
    final_extension = extension.lower().strip('.') if extension and extension != 'n/a' else 'bin'
    ...
    try:
        ...
        # <<< MODIFIED >>> Simplify Referer logic slightly for ArXiv compatibility
        current_headers['Referer'] = http_session.headers.get('Referer', download_url)
        ...
        if response.status_code in [502, 503, 504, 404, 403]:
             ...
             # <<< MODIFIED >>> Generalize error slightly
             return f"Download failed (Server Status: {response.status_code}). The link may be broken or require authentication."
        ...
        # <<< MODIFIED >>> Simplify CAPTCHA check slightly, less Sci-Hub specific now
        if 'text/html' in content_type:
            try:
                peek_bytes = await loop.run_in_executor(None, lambda: response.raw.peek(2048))
                text_snippet = peek_bytes.decode('utf-8', errors='ignore')
                # General check for common interstitial/error indicators
                if 'captcha' in text_snippet.lower() or \
                   'verify you are human' in text_snippet.lower() or \
                   '<form' in text_snippet or \
                   'login' in text_snippet.lower():
                     print(">>> Potential CAPTCHA/Login/Error page detected <<<")
                     response.close()
                     return f"Download failed: Link led to an HTML page (possibly error/login/CAPTCHA). Please visit manually: <{final_url}>"
            except Exception as snippet_err:
                 print(f"  Could not peek/decode HTML snippet: {snippet_err}")

            # If not caught by specific checks but still HTML
            print(">>> DOWNLOAD FAILED: Received HTML instead of expected file type. <<<")
            response.close()
            return f"Download failed: Link led to an unexpected HTML page. Final URL: <{final_url}>"

        # Refine filename/extension based on headers/URL (if needed)
        # <<< MODIFIED >>> Keep extension refinement, useful for both sources
        content_disposition = response.headers.get('content-disposition')
        # <<< MODIFIED >>> Added 'xml' common for ArXiv source files
        valid_extensions = ['pdf', 'epub', 'mobi', 'azw3', 'djvu', 'zip', 'rar', 'txt', 'chm', 'xml']
        ...
        if final_extension in ['bin', 'n/a']:
             type_map = {
                 ...
                 'application/xml': 'xml', 'text/xml': 'xml' # <<< NEW >>> XML type
             }
        ...
        # <<< MODIFIED >>> Check GET content length even if HEAD succeeded, sometimes HEAD lies
        # Use the smaller of the two limits if both exist
        size_to_check = DISCORD_FILE_LIMIT_BYTES
        if content_length and content_length_get:
             size_to_check = min(int(content_length), int(content_length_get))
        elif content_length:
             size_to_check = int(content_length)
        elif content_length_get:
             size_to_check = int(content_length_get)

        if size_to_check > DISCORD_FILE_LIMIT_BYTES:
            print(f"  File size from GET/HEAD header ({size_to_check} bytes) exceeds limit.")
            response.close()
            return f"File is too large ({size_to_check/(1024*1024):.2f} MB). Max size: {DISCORD_FILE_LIMIT_MB} MB."
...
class SearchResultSelectView(View):
    def __init__(self, search_results, original_interaction: discord.Interaction):
        ...
        for i, item in enumerate(options_limited):
            ...
            # <<< MODIFIED >>> Adapt description based on source
            desc_text = "Unknown source"
            source = item.get('source', 'libgen') # Default to libgen if missing
            if source == 'arxiv':
                authors_part = item.get('authors', 'N/A')[:30]
                published_part = item.get('published', 'N/A')
                summary_part = item.get('summary', 'N/A')[:30] # Show just a snippet
                desc_text = f"ArXiv | {authors_part} ({published_part}) | {summary_part}..."
            else: # libgen, fiction, scimag
                authors_part = item.get('authors', 'N/A')[:30]
        ...
        self.select_menu = Select(
            placeholder="Select an item to download (Up to 25 shown)...", # <<< MODIFIED >>> General placeholder
            options=select_options,
            custom_id="item_select_dropdown" # <<< MODIFIED >>> General ID
        )
        self.select_menu.callback = self.select_callback
        self.add_item(self.select_menu)

    # <<< MODIFIED >>> Updated callback method to handle different sources
    async def select_callback(self, interaction: discord.Interaction):
        ...
        source = selected_item.get('source', 'libgen') # <<< NEW >>> Get the source
        ...
        try:
            ...
            if e.code == 40060: print("Interaction already responded to, proceeding with followup.")
            else: print(f"Error editing message to disable select (continuing): {e}")

        # 2. Send a 'thinking' or 'attempting' message using followup
        status_message = None
        try:
            # <<< MODIFIED >>> Slightly more generic thinking message
            ...
            traceback.print_exc()
            try: await interaction.edit_original_response(content="Error starting download process.", view=self)
            except Exception: pass
            return

        # --- Attempt Download based on source ---
        download_result = None
        error_message = "Download failed. Unknown reason."
        direct_download_link = None # Initialize variable

        # <<< NEW >>> ArXiv Download Logic
        if source == 'arxiv':
            pdf_url = selected_item.get('pdf_url')
            if pdf_url:
                print(f"Attempting ArXiv Download: {pdf_url}")
                direct_download_link = pdf_url # ArXiv link is direct
                download_result = await download_book_to_discord(
                    pdf_url,
                    selected_item['title'],
                    'pdf' # ArXiv is always PDF
                )
            else:
                 error_message = "ArXiv result missing PDF URL."
                 print(error_message)

        # <<< EXISTING >>> LibGen Download Logic
        else: # libgen, fiction, scimag
             mirror1_url = selected_item.get('mirror1_url')
             if mirror1_url:
                print(f"Attempting LibGen Mirror 1: {mirror1_url}")
                # Try to find the direct link from the mirror page
                ...
                if direct_download_link:
                    download_result = await download_book_to_discord(
                        ...
                        selected_item['title'],
                        selected_item.get('extension', 'bin') # Use known extension or default
                    )
                else:
        ...
        if isinstance(download_result, discord.File):
            ...
            try:
                 ...
                 try:
                     ...
                     # <<< MODIFIED >>> Offer manual download link based on source
                     manual_link = direct_download_link or selected_item.get('mirror1_url') # Prioritize direct link
                     if manual_link:
                          await interaction.followup.send(f"You can try downloading manually: <{manual_link}>")
                     elif source != 'arxiv' and selected_item.get('mirror2_url'): # Only offer Mirror 2 for Libgen
                          await interaction.followup.send(f"You can try Mirror 2 manually: <{selected_item.get('mirror2_url')}>")
            ...
            if isinstance(download_result, str): error_message = download_result # Use specific error if provided

            print(f"Download failed for '{selected_item['title']}' from source '{source}'. Reason: {error_message}")
            final_error_message = ""

            # --- Failure due to size limit ---
            if error_message.startswith("File is too large"):
                size_info = "size limit"
                match = re.search(r'\((.*MB.*)\)', error_message)
                if match: size_info = match.group(1)
                ...
                # <<< MODIFIED >>> Only offer Mirror 2 for Libgen as backup
                elif source != 'arxiv' and selected_item.get('mirror2_url'):
                    final_error_message += f"\n\nAlternatively, try Mirror 2 manually: <{selected_item.get('mirror2_url')}>"
                elif selected_item.get('details_url'): # Offer details page if other links fail
                     final_error_message += f"\n\nVisit the details page for more options: <{selected_item.get('details_url')}>"

            # --- Other failures ---
            else:
                final_error_message = f"❌ Download failed: {error_message}\n"
                # <<< MODIFIED >>> Adjust alternative suggestions based on source
                if source == 'arxiv':
                    if direct_download_link: # Should be pdf_url
                         final_error_message += f"\nYou can try downloading the PDF manually:\n<{direct_download_link}>"
                    elif selected_item.get('details_url'): # Abstract page link
                         final_error_message += f"\nYou can visit the abstract page:\n<{selected_item.get('details_url')}>"
                else: # Libgen
                    ...
                    elif selected_item.get('details_url') and selected_item.get('details_url') != selected_item.get('mirror1_url'):
                         final_error_message += f"\nYou can visit the details page for more options: <{selected_item.get('details_url')}>"
                    else:
                        final_error_message += "\nNo alternative download options were found for this item."

            # Edit the status message with the error
            try:
                await status_message.edit(content=final_error_message[:2000], attachments=[])
            except discord.NotFound: print("Status message not found during final error edit.")
            except discord.HTTPException as e: print(f"Error editing status message with final error: {e}")
            except Exception as e: print(f"Unexpected error editing status message with final error: {e}")

    # on_timeout remains the same
    async def on_timeout(self):
         ...
         if self.original_interaction and not self.original_interaction.is_expired():
            ...
            except discord.NotFound: print(f"Original interaction message not found on timeout for {self.original_interaction.id}.")
            except discord.HTTPException as e:
                if e.code == 40060: print("Interaction already responded to before timeout edit.")
                else: print(f"HTTP error updating view on timeout for {self.original_interaction.id}: {e}")
            except Exception as e: print(f"Unexpected error updating view on timeout for {self.original_interaction.id}: {e}")
         else: print(f"Original interaction {self.original_interaction.id if self.original_interaction else 'N/A'} expired or missing on timeout.")
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(
        ...
        description="This bot helps you search for books, articles, fiction, and research papers.", # <<< MODIFIED >>>
    ...
    embed.add_field(
        ...
        value=(
            ...
            "2. Enter your search query (title, author, ISBN, DOI, keywords, etc.).\n" # <<< MODIFIED >>>
            "3. Choose a category (**Non-Fiction**, **Fiction**, **Scientific Articles**, **Research Papers**).\n" # <<< MODIFIED >>>
            ...
            f"   - For larger files, a direct download link will be provided.\n" # <<< MODIFIED >>> Simplified explanation
            f"   - If download fails, alternative links may be suggested." # <<< MODIFIED >>>
        ),
        inline=False
    )
    embed.add_field(
        ...
        value=(
            ...
            "This tool is provided for informational purposes and ease of access to publicly available resources."# <<< MODIFIED >>>
    ...
    embed.add_field(
        name="💡 Search Tips",
        value=(
            "- Be specific: Include author names, keywords, or IDs.\n" # <<< MODIFIED >>>
            "- Use ISBN/DOI/ArXiv ID: Provides more exact matches.\n" # <<< MODIFIED >>>
            "- Check Categories: If not found in one, try another relevant category."# <<< MODIFIED >>>
        ),
        inline=False
    )
...
@client.tree.command(name="findbook", description="Search for books, fiction, articles, or papers.") # <<< MODIFIED >>>
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
@app_commands.user_install()
@app_commands.describe(
    query="What to search for (title, author, keywords, ISBN, DOI, ArXiv ID, etc.)", # <<< MODIFIED >>>
    category="Select the search category"
)
@app_commands.choices(category=[ # <<< MODIFIED >>> Added ArXiv
    ...
    app_commands.Choice(name="Research Papers", value="arxiv"), # <<< NEW >>>
...
async def findbook_command(interaction: discord.Interaction, query: str, category: app_commands.Choice[str]):
    ...
    # <<< MODIFIED >>> Perform search based on category
    results = []
    if search_topic == 'arxiv':
         results = await search_arxiv_async(query)
    else: # libgen, fiction, scimag
    ...
    try:
        embed_desc = f"Found {len(results)} results from **{search_topic_name}**. Select one below to attempt download (up to 25 shown)." # <<< MODIFIED >>> Added source name
        embed = discord.Embed(
            title=f"Search Results for '{query}'", # <<< MODIFIED >>> Simplified title
            description=embed_desc[:4090],
            color=discord.Color.green()
        )

        results_to_preview = results[:5]
        added_fields = 0
        for i, item in enumerate(results_to_preview):
             if added_fields >= 5: break

             field_name_text = f"{i+1}. {item['title']}"
             final_field_name = field_name_text[:254] + ".." if len(field_name_text) > 256 else field_name_text

             # <<< MODIFIED >>> Format field value based on source
             field_value_text = "Details unavailable."
             source = item.get('source', 'libgen')
             if source == 'arxiv':
                  authors_val = item.get('authors', 'N/A')[:200]
                  published_val = item.get('published', 'N/A')
                  summary_val = item.get('summary', 'N/A')[:500] # Longer summary in embed
                  ...
                  field_value_text = f"Author(s): {authors_val} ({year_val})\nFormat: {ext_val} | Size: {size_val} | Lang: {lang_val}"
        ...
        if len(results) > 25: footer_text += " Showing the first 25 in the dropdown."
        elif len(results) > added_fields: footer_text += f" Showing {added_fields} in preview."
...
if __name__ == "__main__":
    # <<< NEW >>> Check for arxiv library installation
    try:
        import arxiv
    except ImportError:
        print("###########################################################")
        print("ERROR: 'arxiv' library not found.")
        print("Please install it using: pip install arxiv")
        print("###########################################################")
    ...
    if BOT_TOKEN is None:
        ...
        try:
            ...
            print("Please enable the necessary intents in your bot's application settings.")
            print("###########################################################")
        except Exception as e:
